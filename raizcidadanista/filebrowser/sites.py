    ...
    def _upload_file(self, request):
        ...
        if request.method == "POST":
            ...
            if file_name.split('.')[-1].lower() in ('jpg', 'jpeg'):
                os.system('jpegoptim --strip-all %s' % self.storage.path(file_path))
            elif file_name.split('.')[-1].lower() == 'png':
                os.system('optipng %s' % self.storage.path(file_path))

            os.system('chmod 664 %s' % self.storage.path(file_path))
...
# Default actions
from filebrowser.actions import flip_horizontal, flip_vertical, rotate_90_clockwise, rotate_90_counterclockwise, rotate_180, move
