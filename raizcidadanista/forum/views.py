...
class RecentesView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        topicos_prioritarios_list = Topico.objects.filter(topicoouvinte__ouvinte=self.request.user).distinct().order_by('-dt_ultima_atualizacao')
        topicos_list = list(topicos_prioritarios_list)+list(Topico.objects.filter(topicoouvinte__ouvinte=self.request.user).order_by('-dt_ultima_atualizacao'))
