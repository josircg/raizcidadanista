...
class DateInput5(DateInput):
    ...
    def render(self, name, value, attrs=None):
        ...
        return mark_safe('<div class="input-append datepicker">'
...
class DateRangeInput(MultiWidget):
    ...
    def __init__(self, till_today=True, attrs=None):
        ...
        # TODO: i18n
        end = dict(title=u'Se em branco, é considerada a data de hoje.') \
...
class DateRangeField(MultiValueField):
    ...
    def compress(self, data_list):
        ...
        if not data_list or (not data_list[0] and not data_list[1]):
            ...
            # TODO: i18n
            raise ValidationError(u'Falta preencher a data inicial.')
        elif not data_list[1]:
            if hasattr(self.widget, 'till_today') and self.widget.till_today:
                data_list[1] = date.today()
            else:
                # TODO: i18n
                raise ValidationError(u'Falta preencher a data final.')
        elif data_list[0] > data_list[1]:
            # TODO: i18n
            raise ValidationError(u'A data inicial está posterior à final.')
        return data_list